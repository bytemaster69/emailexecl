...
def save_invalid_to_excel(invalid_emails):
    df = pd.DataFrame(invalid_emails, columns=['Invalid Emails', 'Reason'])
    df.to_excel('invalid_emails.xlsx', index=False)
    print(f"Invalid emails saved to 'invalid_emails.xlsx'")
...
if __name__ == "__main__":
    ...
    if email_list:
        ...
        if valid_emails:
            save_to_excel(valid_emails, 'valid_emails.xlsx')
            print(f"Valid emails saved to 'valid_emails.xlsx'")
        if invalid_emails:
            save_invalid_to_excel(invalid_emails)
